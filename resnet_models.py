...
class ResNet_conv(nn.Module):
    ...
    def _make_layer(
        ...
        stride: int = 1,
        dilate: bool = False,
    ) -> list[Type[Union[BasicBlock, Bottleneck]]]:
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
        ...
        # Remove nn.Sequential so we can get outputs after each block
        return layers
...
class ResNet_conv_with_linear_probes(ResNet_conv):
    def __init__(self, *args, num_classes=10, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        dummy_batch = preprocess(torch.rand((224, 224))).unsqueeze(0)
        outputs = self._forward_impl(dummy_batch)
        self.fc_layers = []
        for output in outputs:
            self.fc_layers.append(nn.Linear(output.shape[1], num_classes))
    
    def forward(self, x: torch.Tensor):
        outputs = self._forward_impl(x)
        return torch.stack([ 
            fc_layer(output)
            for fc_layer, output in zip(self.fc_layers, outputs)
        ], dim=1)
