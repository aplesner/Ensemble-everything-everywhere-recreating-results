...
# Based on `https://pytorch.org/vision/stable/transforms.html#performance-considerations`
...
class Bottleneck(nn.Module):
    ...
    # stride at the first 1x1 convolution(self.conv1)
...
class ResNet_conv(ResNet):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        return x_flat_outputs

    def forward(self, x: Tensor) -> Tensor:
        return self._forward_impl(x)


class ResNet_conv_with_linear_probes(ResNet):
    def __init__(self, *args, num_classes=10, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        dummy_batch = preprocess(torch.rand((224, 224))).unsqueeze(0)
        output_lengths = self._forward_impl_flat(dummy_batch)
        
        self.fc_layers = nn.ModuleList([
            nn.Linear(output_length, num_classes) for output_length in output_lengths
        ])

        del self.fc
    
    def _forward_impl_flat(self, x: torch.Tensor):
        x_flat_output_lengths = [torch.flatten(x, 1).shape[1]]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        return x_flat_output_lengths

    def forward(self, x: torch.Tensor):
        counter = 0
        x_outputs = [self.fc_layers[counter](torch.flatten(x, 1))]
        counter +=1
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def set_resnet_weights(model: torch.nn.Module, weights):
    ...
    for name, param in model.named_parameters():
        if "fc" not in name and name in state_dict:
...
def resnet152_ensemble(num_classes: int=10) -> ResNet_conv_with_linear_probes:
    return ResNet_conv_with_linear_probes(
